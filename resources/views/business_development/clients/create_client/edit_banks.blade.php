...
<div class="container-fluid">
    <div class="btn-group btn-breadcrumb mb-3">
        ...
        <a href="#" style="color:blue;"> Edit Banks</a>
    </div>
    ...
    <div class="row">
        <div class="col-md-12 ">
            ...
            <div class="card px-0 pt-4 pb-0 mt-3 mb-3">
                ...
                <form id="msform" action="{{route('clients.updatebanks')}}" method="POST">
                    ...
                    <fieldset>
                        <div class="form-card">
                            ...
                            <table class="table table-striped">
                                ...
                                <tbody id="bank-div">
                                @foreach($global_customer_id->banks as $bankk)
                                
                                <input type="hidden" name="b_id[]" value="{{$bankk->id}}">
                                    <tr>
                                        <td class="no">1</td>
                                        <td>
                                            <select name="bank_name[]" id="bank_type" class="form-control" required>
                                                <option value="">Select </option>
                                                @foreach ($banks as $bank)
                                                <option value="{{$bank -> bank_code }}"@if($bank->bank_code == $bankk->bank_name)
                                                    selected="selected"
                                                 @endif>
                                                    {{ $bank -> name }}
                                                </option>
                                        ...
                                        <td>
                                            ...
                                            <select name="branch[]" id="branch" class="form-control" required>
                                                ...
                                                <option value="{{$branch -> branch_code}}" @if($branch -> branch_code == $bankk->branch)
                                                    selected="selected"
                                                 @endif>
                                                    {{ $branch -> name }}
                                                </option>
                                        ...
                                        <td>
                                            <input type="text" name="account_name[]" value="{{$bankk->account_name}}" id="account_name"  class="form-control" required />
                                        </td>

                                        <td>
                                            <input type="text" name="account_no[]" id="account_no" value="{{$bankk->account_no}}" class="form-control" required />
                                        </td>
                                        ...
                                        <td>
                                            <select name="status[]" id="status" class="form-control" required>
                                                ...
                                                <option value="{{ $status ->status_description }}" @if($status ->status_description == $bankk->status)
                                                    selected="selected"
                                                 @endif>
                                        ...
                                        <td>
                                            ...
                                            {{-- <select name="default_bank[]" id="default_bank" class="form-control" required>
                                                <option value=1 @if($bankk->default_bank==true)
                                                    selected="selected"
                                                 @endif>Yes</option>
                                                <option value=0  @if($bankk->default_bank==false)
                                                    selected="selected"
                                                 @endif>NO</option>
                                            </select> --}}
                                        </td>
                                        ...
                                        <td>
                                            <button id="deleteBtn" class="btn btn-default btn-small"><span class="fa fa-trash"></span></button>
                                        </td>
                                    </tr>
                                 @endforeach
                                </tbody>
...
<script>
    $(document).ready(function() {
        $("[id=default_bank]").click(function () {
            // alert('clicked')
        });
        ...
        $(function() {
            $('#addBankBtn').click(function() {
                ...
                let tr = '<tr><td class="no">' + n + '</td>' +
                    '<td><select name="addmore['+counter+'][bank_name]" id="bank_type" class="form-control" required>' +
                    '<option value="">Select </option>' +
                    '@foreach ($banks as $bank)' +
                    ...
                    '<td><select name="addmore['+counter+'][branch]" id="branch" class="form-control" required>' +
                    '<option value="">Select </option>' +
                    '@foreach ($branches as $branch)' +
                    ...
                    '<td><input type="text" name="addmore['+counter+'][account_name]" id="account_name" class="form-control" required /></td>' +

                    '<td><input type="text" name="addmore['+counter+'][account_no]" id="account_no" class="form-control" required /></td>' +

                    '<td><select name="addmore['+counter+'][status]" id="status" class="form-control" required>' +
                    '<option value="">Select </option>' +
                    '@foreach ($statuses as $status)' +
                    ...
                    '<td><input type="radio" name="addmore['+counter+'][default_bank]" id="default_bank" value="1"></td>' +
