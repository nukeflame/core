    ...
    @foreach ($reinsurers as $index => $reinsurer)
        @php
            ...
            $portfolioDirection = $filteredCreditItems->contains(function ($item) {
                ...
                ? 'OUT'
                : 'IN';
            $portfolioHeading = 'TREATY PROPORTIONAL ACCOUNT - PORTFOLIO ' . $portfolioDirection;
            $noteHeaderLabel = $portfolioDirection === 'IN' ? 'CREDIT NOTE' : 'DEBIT NOTE';
            $displayAsCredit = $portfolioDirection === 'IN';

            $rawTotalDebit = $filteredCreditItems->filter(fn($item) => $item->ledger === 'DR')->sum('item_amount');
            $rawTotalCredit = $filteredCreditItems->filter(fn($item) => $item->ledger === 'CR')->sum('item_amount');
            $totalDebit = $displayAsCredit ? $rawTotalCredit : $rawTotalDebit;
            $totalCredit = $displayAsCredit ? $rawTotalDebit : $rawTotalCredit;
            $netAmount = abs($totalCredit - $totalDebit);
            ...
            $balanceDueLabel = $displayAsCredit ? 'BALANCE DUE TO YOU' : 'BALANCE DUE FROM YOU';
        @endphp
                    ...
                    <tbody>
                        @foreach (collect($filteredCreditItems)->all() as $item)
                            @php
                                ...
                                $showDebitAmount = $displayAsCredit ? !$isDebitLedger : $isDebitLedger;
