        ...
        <td valign="top">
            <table style="width:60%;">
                ...
                <tr>
                    ...
                    <td style="font-size: 10.0pt; font-family: 'Calibri';">{{ date('Y', strtotime(formatDate($cover->cover_from))) }}-{{ firstUpper($cover->cover_title)}}-{{ firstUpper($cover->customer->name)}}</td>
                </tr>
            </table>
        </td>
        <td>
...
<table style="font-size: 10.0pt; font-family: 'Calibri'; width:100%; margin:0px; border:3px; padding:3px;">
    ...
    <tr>
            <td  align="left" style="font-size: 10.0pt; font-family: 'Calibri'; border: 1px solid #181212; padding: 8px;">
                {{ firstUpper($coverpremium->premium_type_description) }} - Layer {{ firstUpper(str_pad($coverpremium->layer_no, 2, '0', STR_PAD_LEFT)) }} - {{ \Carbon\Carbon::parse($mdpInstallment->installment_date)->format('d/m/Y') }}
            </td>
