...
@push('script')
    <script>
        $(document).ready(function() {
            ...
            $('#refreshEmailsBtn').on('click', () => loadMessages(true));
            ...
            function extractReference(msg) {
                ...
                // Look for patterns like REF-1234, CLAIM-5678, etc.
            ...
            window.handleReply = function(messageId) {
                ...
                $('#to').val(message.from);
                $('#subject').val(message.subject.startsWith('RE:') ? message.subject :
                    `RE: ${message.subject}`);
                $('#category').val(message.category);
                $('#reference').val(message.reference);

                // // Show thread message
                // $('#threadMessage').css('display', 'block');
                // $('#threadMessage').val(
