            ...
            <div id="git_details">
                ...
                <hr>
                    <form id="git_details_form" enctype="multipart/form-data">
                        ...
                        <div class="row m-2">
                            <x-QuotationInputDiv>
                                <x-Input class="git_dec" name="limit_of_liability" id="limit_of_liability"  inputLabel="Limit of Liability" req="required"/>
                            </x-QuotationInputDiv>

                            <x-QuotationInputDiv>
                                <x-Input class="git_dec" name="est_annual_carry" id="est_annual_carry"  inputLabel="Estimated Annua Carry" req="required"/>
                            </x-QuotationInputDiv>
                            
                            <x-QuotationInputDiv>
                                <x-SearchableSelect class="git_dec" name="package_type" id="package_type" req="required" inputLabel="Package Type">
                                    <option value="">Select package type</option>
                                    <option value="P">Packed Goods</option>
                                    <option value="U">Unpacked Goods</option>
                                </x-SearchableSelect>
                            </x-QuotationInputDiv>
                        ...
                        <div class="row">
                            ...
                            <x-QuotationInputDiv>
                                <x-Input class="git_dec" name="veh_reg_no" id="veh_reg_no"  inputLabel="Vehicle Reg No" req=""/>
                            </x-QuotationInputDiv>
                            
                            <x-QuotationInputDiv>
                                <x-NumberInput class="git_dec" name="veh_value" id="veh_value"  inputLabel="Value Of Vehicle" req=""/>
                            </x-QuotationInputDiv>
                            
                            <x-QuotationInputDiv>
                                <x-NumberInput class="git_dec" name="veh_max_load" id="veh_max_load"  inputLabel="Maximum Load for Vehicle. (kgs)" req=""/>
                            </x-QuotationInputDiv>
                        </div>
                ...
                <div id="gits" class="table-responsive">
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                <th>Goods Description</th>
                                <th>One Off Transit</th>
                                <th>Road</th>
                                <th>Rail</th>
                                <th>Annual Carry</th>
                                <th>Action</th>
                        ...
                        <tbody>
                            @foreach($gits as $git)
                                <tr>
                                    <td>{{ $git->goods_descr }}</td>
                                    <td>{{ $git->plan }}</td>
                                    <td>{{ $git->road_transport }}</td>
                                    <td>{{ $git->rail_transport }}</td>
                                    <td>{{ number_format($git->annual_carry, 2) }}</td>
                                    <td>
                                        
                                        <span type="span" class='text-primary editdetails' data-quote='{{$git->quote_no}}' title="Edit" style="cursor:pointer;">
                                            <a href="{{ route('show_git_details', ['qstring'=>Crypt::encrypt('quote_no='.$git->quote_no.'&source='.$source)])}}">
                                                <i class='fa fa-edit'></i>Edit
                                            </a>
                                        </span>|
                                        <span class="text-primary" title="Add section">
                                            <a href="{{route('add_location_sections',['qstring'=>Crypt::encrypt('quote_no='.$git->quote_no.'&source='.$source.'&location=1')])}}" class="location_det" style="text-decoration: none">
                                                    <i class='fa fa-circle-plus'></i>Section Details
                                            </a>
                                        </span>
                                    </td>
                                </tr>
