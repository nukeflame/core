...
class FirstOutlookService
{
    ...
    public function refreshToken(): bool
    {
        try {
            ...
            $response = Http::asForm()->timeout($this->timeout)->post($tokenUrl, [
    ...
    public function getUserPhoto($user, string $size = '96x96'): ?string
    {
        try {
            ...
            Log::info('Profile photo not available: ' . $e->getMessage());
        }
        return null;
    }
    ...
    public function sendEmail($auth, array $emailData): array
    {
        try {
            ...
            if (empty($emailData['to']) || empty($emailData['subject'])) {
                throw new Exception('Missing required fields: to and subject are required');
            }
            ...
            Log::info('Sending email via Outlook API', [
                'user' => $auth->email,
                'subject' => $emailData['subject'],
    ...
    public function sendEmailWithMessageId($auth, array $emailData): array
    {
        try {
            ...
            Log::error('Failed to send email with message ID', [
                'error' => $e->getMessage(),
                'subject' => $emailData['subject'] ?? 'N/A'
            ]);
    ...
    public function sendReply($auth, string $originalMessageId, array $replyData): array
    {
        try {
            ...
            if (!empty($replyData['customHeaders'])) {
                $replyMessage['message']['internetMessageHeaders'] = $replyData['customHeaders'];
            }

            if (!empty($replyData['priority'])) {
                $replyMessage['message']['importance'] = $this->mapPriorityToImportance($replyData['priority']);
            }
    ...
    public function sendReplyAll($auth, string $originalMessageId, array $replyData): array
    {
        try {
            ...
            Log::info('Sending reply all', [
                'original_message_id' => $originalMessageId,
                'conversation_id' => $originalMessage['conversationId'],
                'to_recipients_count' => count($originalMessage['toRecipients'] ?? []),
                'cc_recipients_count' => count($originalMessage['ccRecipients'] ?? []),
                'user' => $auth->email
            ]);
    ...
    public function canReplyToMessage(string $messageId): array
    {
        try {
            ...
            $isDraft = str_contains(strtolower($message['parentFolderId'] ?? ''), 'draft');
            $isOutbox = str_contains(strtolower($message['parentFolderId'] ?? ''), 'outbox');
            $isDeleted = str_contains(strtolower($message['parentFolderId'] ?? ''), 'deleted');

            if ($isDraft) {
                return [
                    'can_reply' => false,
                    'reason' => 'Cannot reply to draft messages'
                ];
            }
            ...
            if ($isOutbox) {
                return [
                    'can_reply' => false,
                    'reason' => 'Cannot reply to messages in outbox'
                ];
            }
            ...
            if ($isDeleted) {
                return [
                    'can_reply' => false,
                    'reason' => 'Cannot reply to deleted messages'
                ];
            }
    ...
    /**
     * Fetch emails from a specific folder with caching
     */
    ...
    public function markMessage(string $messageId, bool $isRead = true): bool
    {
        try {
            ...
            Log::error('Failed to mark message: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public function moveMessage(string $messageId, string $destinationFolderId): bool
    {
        try {
            ...
            Log::error('Failed to move message: ' . $e->getMessage());
            return false;
        }
    }
    ...
    public function createDraft(array $emailData): ?array
    {
        try {
            ...
            Log::error('Failed to create draft: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function revokeAuthentication(): bool
    {
        try {
            ...
            Log::error('Failed to revoke authentication: ' . $e->getMessage());
            return false;
        }
    }
    ...
    private function processEmailsBulk(array $rawEmails): array
    {
        ...
        foreach ($chunks as $chunk) {
            ...
            if (count($chunks) > 1) {
                usleep(1000); // Brief pause between chunks
            }
        }
    ...
    /**
     * Set authentication user (for queue jobs)
     */
