                    ...
                    if ($coverdebit) {
                        ...
                        $partnerItems = ReinNote::where('endorsement_no', $coverdebit->endorsement_no)
                            ...
                            ->map(function ($itemsByPremiumType, $premiumTypeDescription) use ($coverReg, &$countItem) {
                                $countItem++;
                                $net_amt = $itemsByPremiumType->sum('net_amt');
                        ...
                        //Premium
                        $PremiumDebit = CoverPremium::where('endorsement_no', $coverdebit->endorsement_no)
                            ->where('dr_cr', 'DR')
                            ->sum('final_amount') ?? 0;
                        $PremiumCredit = CoverPremium::where('endorsement_no', $coverdebit->endorsement_no)
                            ->where('dr_cr', 'CR')
                            ->sum('final_amount') ?? 0;
                        $premiumAmt = abs($PremiumCredit - $PremiumDebit);
                        $countItem++;
                        $premiumItem = [
                            'item_number' => (string)$countItem,
                            'item_name' => 'PRM',
                        ...
                        $otherItem = [
                            ...
                            'local_nett_total_amount' => ($otherIncome * ($coverReg->currency_rate ?: 1)),
                            'quantity' => 1,
                            'status' => 'A',
                            'dr_cr' =>  $coverdebit->document === 'DRN' ? 'C' : 'D'
                        ...
                        $requestPayload = [
                            'invoice' => [
                                ...
                                'dr_cr' => substr($coverdebit->document, 0, 1),
                                'invoice_due_date' => $coverdebit->premium_payment_due_date,
                                'local_amount' => $premiumAmt * $coverReg->currency_rate ? $coverReg->currency_rate : 1,
                                'notes' => $coverdebit->document . ' document',
                                'terms_conditions' => null,
                                'created_by' => $coverdebit->created_by ?? 'system',
                                'updated_by' => $coverdebit->updated_by ?? 'system',
                                'created_at' => $coverdebit->created_at ?? now(),
                                'updated_at' => $coverdebit->updated_at ?? now(),
                                'approved_by' =>  $approval->updated_by ?? 'system',
                                'approved_at' => $approval->updated_at ?? now(),
                                'type_of_bus' => $coverReg->type_of_bus,
                                'transaction_type' => $coverdebit->document,
                                'invoice_status' => 'A',
                                'subsidiary_code' => 'REINSURANCE',
                                'foreign_amount' => $premiumAmt,
                                ...
                                'invoice_title' => (in_array($coverReg->type_of_bus, ['FPR', 'FNP'])
                        ...
                        if ($response->successful()) {
                            ...
                            CoverDebit::where('endorsement_no', $data->endorsement_no)->update([
                                'gl_updated' => 'Y',
                                'gl_updated_retries' => $coverdebit->gl_updated_retries + 1,
                            ...
                            $this->error("Error for Endorsement No {$data->endorsement_no}: $formattedErrors");
                        }
                    }
                }
            } else {
