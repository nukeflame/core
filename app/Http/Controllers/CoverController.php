...
use Nukeflame\Core\Services\CreditNoteService;
...
use Nukeflame\Core\Services\DebitNoteService;
...
class CoverController extends Controller
{
        ...
        $customers = DB::table('customers')
            ->join('customer_types', function ($join) {
                $join->on('customer_types.type_id', '=', DB::raw(
                    "ANY (
                        CASE
                            WHEN jsonb_typeof(to_jsonb(customers.customer_type)) = 'array'
                                THEN ARRAY(SELECT jsonb_array_elements_text(to_jsonb(customers.customer_type))::int)
            ...
            foreach ($request->treaty as $treatyIndex => $treaty) {
                foreach ($treaty['reinsurers'] as $reinsurerIndex => $reinsurerData) {
                    ...
                    if (in_array($coverRegister->type_of_bus, ['FPR', 'FNP'])) {
                        ...
                        $netOfTax = !empty($reinsurerData['net_of_tax']) ? 1 : 0;

                        $coverRegister->commission_mode = $netOfTax ? 'net' : 'gross';
                        ...
                        $coverRipart->commission_mode = $netOfTax ? 'net' : 'gross';
                        $coverRipart->net_amount = 0;
                        $coverRipart->net_of_tax = $netOfTax;
    ...
    private function createCustomerAccount(array &$debitData, CoverRegister $coverRegister, $treatyDebit, $treatyCredit, $ref = ''): void
    {
        ...
        if (! $isPortfolio && empty($debitData['drCrNo'])) {
            $debitData['drCrNo'] = SystemSerials::nextSerial($debitData['docType']);
        }

        $now = Carbon::now();
        $postingDate = ! empty($debitData['postingDate']) ? Carbon::parse($debitData['postingDate']) : $now;
        ...
        $custAccount->currency_code = $isPortfolio
    ...
    public function StorePropPortfolio(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'status' => Response::HTTP_CREATED,
                'message' => 'Portfolio transaction generated successfully',
                'endorsement_no' => $cover->endorsement_no,
                'computed' => [
                    ...
                    'profit_comm_rate' => $profit_comm_rate,
                ],
            ], Response::HTTP_CREATED);
        } catch (ValidationException $e) {
            logger($e);
