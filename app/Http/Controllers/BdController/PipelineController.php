...
class PipelineController
{
    ...
    private function getActionButtons($opp)
    {
        $id = (int) $opp->id;
        ...
        try {
            ...
            $results = collect($reinsurers['data'])->map(function ($reinsurer) use ($countries) {
                return [
                    ...
                    'name' => Str::upper($reinsurer['name']),
                    'capacity' => $reinsurer['capacity'] ?? '',
                    'rating' => $reinsurer['rating'] ?? '',
                    'email' => $reinsurer['email'],
                    'country' => $countries[$reinsurer['country_iso']] ?? null,
                    'contacts' => $reinsurer['contacts'],
                ];
            })->sortBy('name')->values();
            ...
            return response()->json([
                'results' => $results,
                'pagination' => [
                    'more' => $reinsurers['has_more'] ?? false
                ]
            ]);
        } catch (\Exception $e) {
            return response()->json([
    ...
    private function getReinsurersData(?string $search, int $page, int $perPage, $cedantId, $stage, $oppId): array
    {
        ...
        ')
            ...
            ->when(
                ...
                $q->where(
                    fn($subQ) =>
                    $subQ->where('customers.name', 'ILIKE', "%{$search}%")
                        ->orWhere('customers.email', 'ILIKE', "%{$search}%")
                        ->orWhere('customers.city', 'ILIKE', "%{$search}%")
                )
            )
        ...
        $total = $results->first()?->total_count ?? 0;

        $data = $results->map(function ($row) {
            $customer = (array) $row;
            unset($customer['total_count']);

            $customer['contacts'] = $row->contacts
    ...
    public function updateLeadStatus(Request $request)
    {
        ...
        try {
            ...
            if (!empty($reinsurers) && is_array($reinsurers)) {
                foreach ($reinsurers as $index => $rein) {
                    try {
                        ...
                        $writtenShare = $rein['written_share'] ?? null;
                        ...
                        DB::table('bd_fac_reinsurers')->updateOrInsert(
                            [
                                'reinsurer_id' => $reinsurerId,
                                'opportunity_id' => $opportunityId,
                            ],
                            [
                                'reinsurer_name' => $customerName,
                                'email' => $email,
                                'share_amount' => $writtenShare * $premium / 100,
            ...
            if (!empty($updateData)) {
                $updated = DB::table('pipeline_opportunities')
                    ->where('opportunity_id', $prospect->opportunity_id)
                    ->update($updateData);
