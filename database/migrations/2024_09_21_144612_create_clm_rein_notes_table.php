<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('claim_rein_notes', function (Blueprint $table) {
            $table->decimal('tran_no',20,0)->nullable(false)->primary();
            $table->string('cover_no',20)->nullable(false);
            $table->string('endorsement_no',20)->nullable(false);
            $table->string('claim_no',20)->nullable(false);
            $table->decimal('dr_no',9,0)->nullable(false);
            $table->string('item_title',100)->nullable(false)->default(' ');
            $table->string('dr_cr',3)->nullable(false);
            $table->string('transaction_type',3)->nullable(false);
            $table->string('entry_type_descr',3)->nullable(false);
            $table->decimal('partner_no',11,0)->nullable(false);
            $table->decimal('share',8,5)->nullable(false);
            $table->decimal('total_gross',20,2)->nullable(false);
            $table->decimal('gross',20,2)->nullable(false);
            $table->decimal('net_amt',20,2)->nullable(false);
            $table->decimal('rate',8,5)->nullable(false);
            $table->decimal('ln_no',10,0)->nullable(false);
            $table->decimal('account_year',4,0)->nullable(false);
            $table->decimal('account_month',2,0)->nullable(false);
            $table->string('created_by',20)->nullable(false);
            $table->string('updated_by',20)->nullable(false);
            $table->timestamps();
            $table->index(['claim_no', 'tran_no'], 'claim_rein_notes_1');
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('claim_rein_notes');
    }
};
